...
constexpr int
get_next_row(int row, int col) 
{
    return row + (col + 1) / 9;
}

constexpr int
get_next_col(int col) 
{
    return (col + 1) % 9;
}
...
private:
    ...
    next_empty_position(vector<vector<char>> &board, int row, int col) 
    {
        while (row != 9)
        {
            if (board[row][col] == '.')
    ...
    static bool
    solve(vector<vector<char>> &board, int const row_start, int const col_start,
